...
import { buildMobileToolbarMenuGridDelegate } from '../utils/mobile_toolbar_menu_grid_delegate';
import { MobileToolbarTheme } from '../mobile_toolbar_style';

export const textDecorationMobileToolbarItem = MobileToolbarItem.withMenu({
  ...
  itemMenuBuilder: (editorState, service) => {
    ...
    return new TextDecorationMenu(editorState, selection).render();
  }
});

class TextDecorationMenu {
  ...
  private updateButtonState(container: HTMLElement): void {
    // Simple re-render approach - in a more sophisticated implementation
    // you'd update individual button states
