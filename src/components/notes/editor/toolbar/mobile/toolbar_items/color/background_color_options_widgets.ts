...
import { generateHighlightColorOptions, formatHighlightColor } from '../../../../util/color_util';

export class BackgroundColorOptionsWidgets {
  ...
  constructor(
    ...
    selection: Selection,
    options: { backgroundColorOptions?: ColorOption[] } = {}
  ) {
  ...
  render(): HTMLElement {
    ...
    const colorOptions = this.backgroundColorOptions ?? generateHighlightColorOptions();
    const selection = this.selection;
    const nodes = this.editorState.getNodesInSelection(selection);
    const hasBackgroundColor = nodes.allSatisfyInSelection(selection, (delta) => {
    ...
    colorOptions.forEach(colorOption => {
      const isSelected = nodes.allSatisfyInSelection(selection, (delta) => {
        return delta.everyAttributes(
          (attributes) => attributes[AppFlowyRichTextKeys.backgroundColor] === colorOption.colorHex
        );
      });
