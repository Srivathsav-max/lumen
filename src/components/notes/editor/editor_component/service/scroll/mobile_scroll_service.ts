...
export class MobileScrollService implements AppFlowyScrollService {
  ...
  private createElement(child: HTMLElement): HTMLElement {
    ...
    container.className = 'mobile-scroll-service';
  ...
  scrollTo(dy: number, options: { duration?: number } = {}): void {
    ...
    this.editorScrollController.scrollOffsetController.animateScroll({
      offset: clampedDy,
      duration,
    });
  }
  ...
  jumpTo(index: number): void {
    this.editorScrollController.itemScrollController.jumpTo({ index, alignment: 0 });
  }

  jumpToTop(): void {
    // Mobile implementation - can be empty or implement specific behavior
  }
