...
func (r *KnowledgeDocumentRepository) Create(ctx context.Context, d *repository.KnowledgeDocument) error {
	query := `
        INSERT INTO knowledge_documents (id, user_id, workspace_id, appwrite_bucket_id, appwrite_file_id, original_filename, mime_type, size_bytes, status, page_count, metadata, created_at, updated_at)
        VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, COALESCE($8,'uploaded'), $9, COALESCE($10::jsonb,'{}'::jsonb), NOW(), NOW())
        RETURNING id, user_id, workspace_id, appwrite_bucket_id, appwrite_file_id, original_filename, mime_type, size_bytes, status, page_count, metadata, created_at, updated_at
    `
	...
	row := r.ExecuteQueryRow(ctx, query, d.UserID, d.WorkspaceID, d.AppwriteBucketID, d.AppwriteFileID, d.OriginalFilename, d.MimeType, d.SizeBytes, d.Status, d.PageCount, metaParam)
	return row.Scan(&d.ID, &d.UserID, &d.WorkspaceID, &d.AppwriteBucketID, &d.AppwriteFileID, &d.OriginalFilename, &d.MimeType, &d.SizeBytes, &d.Status, &d.PageCount, &d.Metadata, &d.CreatedAt, &d.UpdatedAt)
}
...
func (r *KnowledgeDocumentRepository) GetByID(ctx context.Context, id string) (*repository.KnowledgeDocument, error) {
	query := `
        SELECT id, user_id, workspace_id, appwrite_bucket_id, appwrite_file_id, original_filename, mime_type, size_bytes, status, page_count, metadata, created_at, updated_at
        FROM knowledge_documents WHERE id = $1
    `
	row := r.ExecuteQueryRow(ctx, query, id)
	d := &repository.KnowledgeDocument{}
	if err := row.Scan(&d.ID, &d.UserID, &d.WorkspaceID, &d.AppwriteBucketID, &d.AppwriteFileID, &d.OriginalFilename, &d.MimeType, &d.SizeBytes, &d.Status, &d.PageCount, &d.Metadata, &d.CreatedAt, &d.UpdatedAt); err != nil {
...
func (r *KnowledgeDocumentRepository) ListByWorkspace(ctx context.Context, workspaceID int64, limit, offset int) ([]*repository.KnowledgeDocument, error) {
	...
	for rows.Next() {
		d := &repository.KnowledgeDocument{}
		if err := rows.Scan(&d.ID, &d.UserID, &d.WorkspaceID, &d.AppwriteBucketID, &d.AppwriteFileID, &d.OriginalFilename, &d.MimeType, &d.SizeBytes, &d.Status, &d.PageCount, &d.Metadata, &d.CreatedAt, &d.UpdatedAt); err != nil {
